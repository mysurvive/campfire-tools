...
async function getItemNames(uuids: Set<string>): Promise<Set<string>> {
  const applicableItems = (
    (await Promise.all(
      Array.from(uuids).map((uuid) => fromUuid(uuid)),
    )) as (Item | null)[]
...
function getItemsByName(
  ...
  return actor.itemTypes[type]?.filter((i) => names.has(i.name)) as Item[];
}

export async function getActorItemsByUuid(
  actor: Actor,
  type: ItemType,
  uuids: Set<string>,
): Promise<Item[]> {
...
export async function getActorEffectsByUuid(
  actor: Actor,
  uuids: Set<string>,
): Promise<(Item & EffectItem)[]> {
  const items = await getActorItemsByUuid(actor, "effect", uuids);
  return items.filter(isEffectItem);
}

export async function actorHasEffectByUuid(
  actor: Actor,
  uuid: string,
): Promise<boolean> {
...
export async function getConsumablesByUuid(
  actor: Actor,
  uuids: Set<string>,
): Promise<(Item & ConsumableItem)[]> {
  const items = await getActorItemsByUuid(actor, "consumable", uuids);
  return items.filter(isConsumableItem);
}

export async function hasItemByUuid(
  actor: Actor,
  type: ItemType,
  uuids: Set<string>,
): Promise<boolean> {
...
export async function validateSkillProficiencies(
  actor: Actor,
  ...
  for (const requirement of requirements) {
    ...
    if (!(await satisfiesSkillProficiency(actor, skill, proficiency))) {
      ...
      throw new NotProficientError(
        `${name} requires at least ${capitalize(proficiency)} in ${unslugify(
          skill,
        )}`,
      );
    }
...
export async function satisfiesSkillProficiency(
  actor: Actor,
  skill: string,
...
export async function getActorsByUuid(uuids: Set<string>): Promise<Actor[]> {
  return (
    await Promise.all(Array.from(uuids).map((a) => getActorByUuid(a)))
...
export async function getItemsByUuid(uuids: Set<string>): Promise<Item[]> {
  return (
    await Promise.all(Array.from(uuids).map((i) => getItemByUuid(i)))
  ).filter((a) => a !== null) as Item[];
}

export async function getEffectByUuid(
  uuid: string,
): Promise<(Item & EffectItem) | null> {
  const document = await fromUuid(uuid);
...
export async function getEffectsByUuid(
  uuids: Set<string>,
): Promise<(Item & EffectItem)[]> {
...
async function removeActorEffectsByUuid(
  actor: Actor,
  uuids: Set<string>,
): Promise<void> {
  ...
  await actor.deleteEmbeddedDocuments(
    "Item",
    existingEffects.map((e) => e.id),
  );
}

async function removeActorEffectsByName(
  actor: Actor,
  names: Set<string>,
): Promise<void> {
...
export async function removeActorsEffectsByUuid(
  actors: Actor[],
  uuids: Set<string>,
): Promise<void> {
...
export async function removeActorsEffectsByName(
  actors: Actor[],
  names: Set<string>,
): Promise<void> {
