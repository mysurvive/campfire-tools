  ...
  return {
    ...
    cooking: {
      ...
      knownRecipes: Array.from(
        new Set(["Basic Meal", "Hearty Meal", ...migratedRecipes]),
      ),
      homebrewMeals: [],
    ...
    restRollMode: "one",
    currentRegion: "Yarrin Southern Plains",
...
export function getDC(actor: Actor, dcType: DcType, region: string): number {
  if (dcType === "zone") {
    return getRegionInfo(region).zoneDC;
...
export async function rollCampingCheck({
  ...
  if (dc) {
    rollData["dc"] = { value: getDC(actor, dc, region) };
  }
  if (secret) {
  ...
  const skillToRoll =
    skill in skills ? skill : loreSkill in skills ? loreSkill : null;
  if (skill === "perception") {
    result = await actor.perception.roll(rollData);
  } else if (skillToRoll === null) {
    ui.notifications?.error(
      `${actor.name} does not have skill ${unslugify(skill)}`,
    );
    return null;
  } else {
  ...
  if (degree !== null) {
    ...
    if (
      activity !== undefined &&
      activity[degreeToProperty(degree)]?.checkRandomEncounter
    ) {
...
export function getCampingActivityData(
  current: Camping,
): CampingActivityData[] {
...
const combatEffects: Partial<Record<CampingActivityName, CombatEffect>> = {
  "Enhance Weapons": {
    uuid: "@UUID[Compendium.campfire-tools.kingmaker-tools-camping-effects.ZKJlIqyFgbKDACnG]{Enhance Weapons}",
    target: "Allies",
  },
  "Set Traps": {
...
export function getCombatEffects(
  data: Camping,
): Partial<Record<CampingActivityName, CombatEffect>> {
...
export async function getHuntAndGatherActor(
  data: Camping,
): Promise<Actor | null> {
