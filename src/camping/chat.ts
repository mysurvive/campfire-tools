...
async function parseIngredientButton(
  element: HTMLElement,
): Promise<ActorAndIngredients | null> {
  const actorUuid = element.dataset["actorUuid"] ?? "";
  const specialIngredients = parseInt(
    element.dataset["specialIngredients"] ?? "0",
  );
...
export async function addRecipe(
  ...
  if (actorAndIngredients) {
    if (isFirstGm(game)) {
      ...
      game.socket?.emit("module.campfire-tools", {
...
export async function postHuntAndGatherResult(
  actor: Actor,
  food: FoodAmount,
): Promise<void> {
...
export async function postDiscoverSpecialMealResult(
  ...
  const content = `
        <p>${recipe ? `Learn ${recipe} and remove` : "Remove"}${
          critFailUuids.length > 0
            ? " and suffer the Critical Failure effect"
            : ""
        }:</p>
...
export async function checkRandomEncounterMessage(): Promise<void> {
  await ChatMessage.create({
    content:
      '<button class="km-random-encounter" type="button">Check for Random Encounter</button>',
...
export function bindCampingChatEventListeners(game: Game): void {
  ...
  if (actor) {
    ...
    chatLog.on("click", ".km-add-food", (event) =>
      addIngredients(game, event.currentTarget),
    );
    chatLog.on("click", ".km-add-recipe", (event) =>
      addRecipe(game, event.currentTarget),
    );
    chatLog.on("click", ".km-random-encounter", () =>
      checkForRandomEncounter(game),
    );
  }
}
