        ...
        <form class="simple-dialog-form">
            ...
            ${["critical-success", "success", "failure", "critical-failure"]
              .map((d) => {
                return `
                  <div>
                    <label for="km-${d}-effect">${unslugify(
                      d,
                    )}: Effect UUID</label>
                    <input type="text" name="${d}-effect" id="km-${d}-effect">
                  </div>
                  <div>
                    <label for="km-${d}-effect-target">${unslugify(
                      d,
                    )}: Effect Target</label>
                  ...
                  <div>
                    <label for="km-${d}-message">${unslugify(
                      d,
                    )}: Message</label>
                    <textarea name="${d}-message" id="km-${d}-message" placeholder=""></textarea>
                  </div>
                  <div>
                    <label for="km-${d}-day-modifier">${unslugify(
                      d,
                    )}: Day Encounter DC Modifier</label>
                    <input type="number" name="${d}-day-modifier" id="km-${d}-day-modifier" placeholder="0">
                  </div>
                  <div>
                    <label for="km-${d}-night-modifier">${unslugify(
                      d,
                    )}: Night Encounter DC Modifier</label>
                    <input type="number" name="${d}-night-modifier" id="km-${d}-night-modifier" placeholder="0">
                  </div>
                  <div>
                    <label for="km-${d}-roll-encounter">${unslugify(
                      d,
                    )}: Random Encounter</label>
        ...
        add: {
          ...
          callback: async (html): Promise<void> => {
            ...
            const name = (parseTextInput($html, "name") ||
              "Unknown Activity") as CampingActivityName;
            const skillRequirements = parseSkillList(
              parseTextInput($html, "skill-requirement"),
            );
            const skills = parseSkills(parseTextInput($html, "skills"));
            if (journalUuid && journal === null) {
              ui.notifications?.error(
                `Can not find journal with uuid ${journalUuid}`,
              );
            } else if (effectUuid && effectItem === null) {
              ui.notifications?.error(
                `Can not find effect item with uuid ${effectUuid}`,
              );
            } else if (activities.find((r) => r.name === name)) {
              ui.notifications?.error(
                `Activity with name ${name} exists already`,
              );
            } else if (!skillRequirements.every((s) => slugifyable(s))) {
              ui.notifications?.error(
                `Skill Requirements "${skillRequirements}" can not be slugified`,
              );
            } else if (
              skills !== "any" &&
              !skills.every((s) => slugifyable(s))
            ) {
              ui.notifications?.error(
                `Skills "${skills}" can not be slugified`,
              );
            } else {
              const skillProficiency = parseProficiency(
                parseSelect($html, "skill-proficiency"),
              );
              await onSubmit({
                name,
                journalUuid: journalUuid,
                skills:
                  skills === "any" ? "any" : skills.map((s) => slugify(s)),
                skillRequirements: skillProficiency
                  ? skillRequirements.map((s) => {
                      return {
                        skill: slugify(s),
                  ...
                  ? [
                      {
                        uuid: effectUuid,
                        targetAll:
                          parseSelect($html, "effect-target") === "all",
                      },
                    ]
                  : undefined,
...
function parseSkillList(value: string): string[] {
  if (value.trim()) {
    return value
      .split(",")
      .map((s) => s.toLowerCase().replace("lore", "").trim());
  }
  return [];
}
...
async function parseEffects(
  $html: HTMLElement,
  name: "critical-success" | "success" | "failure" | "critical-failure",
