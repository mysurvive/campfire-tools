          ...
          callback: async (html): Promise<void> => {
            ...
            if (item === null) {
              ...
              ui.notifications?.error(
                `Recipe with name ${name} exists already`,
              );
            } else {
              const favoriteEffect = await parseEffects($html, "favorite-meal");
              const critSuccEffect = await parseEffects(
                $html,
                "critical-success",
              );
              ...
              await onSubmit({
                isHomebrew: true,
                specialIngredients:
                  parseNumberInput($html, "special-ingredients") || 0,
                ...
                basicIngredients:
                  parseNumberInput($html, "basic-ingredients") || 0,
