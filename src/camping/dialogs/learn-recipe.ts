...
function tpl(
  availableRecipes: ViewRecipeData[],
  availableFood: FoodAmount,
): string {
  const recipes = availableRecipes.map((r) => {
    return {
      ...r,
      disabled:
        availableFood.specialIngredients < r.specialIngredients * 2 ||
  ...
  return `
    <form>
        ...
        <p><b>Available Basic Ingredients</b>: ${
          availableFood.basicIngredients
        }</p>
        <p><b>Available Special Ingredients</b>: ${
          availableFood.specialIngredients
        }</p>
            ...
            ${recipes
              .map((r, index) => {
                ...
                return `<tr>
                    ...
                    <td><input type="radio" name="recipe" value="${escapeHtml(
                      r.name,
                    )}" ${checked ? "checked" : ""} ${
                      r.disabled ? "disabled" : ""
                    }></td>
...
export async function discoverSpecialMeal(
  options: LearnRecipeOptions,
): Promise<void> {
  const availableRecipes = await Promise.all(
    options.availableRecipes.map((r) => toTemplateRecipe(r, new Set())),
  );
  new Dialog(
    {
