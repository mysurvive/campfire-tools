...
interface ManageActivitiesOptions {
  ...
  onSubmit: (
    lockedActivities: Set<CampingActivityName>,
    deletedActivities: Set<CampingActivityName>,
  ) => Promise<void>;
  isGM: boolean;
}
                ...
                ${data
                  .map((r, i) => {
                    return `<tr>
                        ...
                        <td><input type="checkbox" name="lock-${i}" ${
                          !lockedActivities.has(r.name) ? "checked" : ""
                        }></td>
                        <td><input type="checkbox" name="delete-${i}" ${
                          !isGM || !r.isHomebrew ? "disabled" : ""
                        }></td>                      
                    </tr>`;
                  })
                  .join("\n")}
...
export async function manageActivitiesDialog(
  options: ManageActivitiesOptions,
): Promise<void> {
        ...
        save: {
          ...
          callback: async (html): Promise<void> => {
            ...
            await options.onSubmit(
              new Set(lockedActivities),
              new Set(deletedActivities),
            );
          },
        },
      },
