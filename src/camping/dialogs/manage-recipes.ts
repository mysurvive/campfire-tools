...
export async function toTemplateRecipe(
  recipe: RecipeData,
  learnedRecipes: Set<string>,
): Promise<ViewRecipeData> {
  return {
    learned: learnedRecipes.has(recipe.name),
    name: recipe.name,
    recipe: await TextEditor.enrichHTML(
      createUUIDLink(recipe.uuid, recipe.name),
    ),
    recipeName: recipe.name,
    cookingLoreDC: recipe.cookingLoreDC,
    cookingCost: formatCost(recipe.basicIngredients, recipe.specialIngredients),
    learningCost: formatCost(
      recipe.basicIngredients * 2,
      recipe.specialIngredients * 2,
    ),
                ...
                ${data
                  .map((r, index) => {
                    return `<tr>
                        ...
                        <td><input type="checkbox" name="add-${index}" ${
                          r.learned ? "checked" : ""
                        } ${
                          r.name === "Basic Meal" || r.name === "Hearty Meal"
                            ? "disabled"
                            : ""
                        }></td>
                        <td><input type="checkbox" name="delete-${index}" ${
                          r.isHomebrew && isGM ? "" : "disabled"
                        }></td>
                    </tr>`;
                  })
                  .join("\n")}
...
export async function manageRecipesDialog(
  options: ManageRecipesOptions,
): Promise<void> {
  const recipes = [...options.recipes];
  recipes.sort((a, b) => a.level - b.level || a.name.localeCompare(b.name));
  const data = await Promise.all(
    recipes.map((r) => toTemplateRecipe(r, options.learnedRecipes)),
  );
  new Dialog(
    {
        ...
        save: {
          ...
          callback: async (html): Promise<void> => {
            ...
            await options.onSubmit(
              new Set(enableRecipes),
              new Set(deleteRecipes),
            );
          },
        },
      },
