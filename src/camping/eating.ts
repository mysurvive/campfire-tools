...
export async function getConsumableFromUuid(
  uuid: string,
): Promise<(Item & ConsumableItem) | null> {
...
export async function getActorConsumables(
  actors: Actor[],
): Promise<FoodAmount> {
  const rationSourceId = (await getConsumableFromUuid(rationUuid))?.sourceId;
  const specialIngredientsSourceId = (
    await getConsumableFromUuid(specialIngredientUuid)
  )?.sourceId;
  const basicIngredientsId = (await getConsumableFromUuid(basicIngredientUuid))
    ?.sourceId;
  ...
  for (const actor of actors) {
    ...
    const ration = consumables?.find(
      (c) => c.sourceId === rationSourceId && rationSourceId !== undefined,
    ) as ConsumableItem | undefined;
    const specialIngredient = consumables?.find(
      (c) =>
        c.sourceId === specialIngredientsSourceId &&
        specialIngredientsSourceId !== undefined,
    ) as ConsumableItem | undefined;
    const basicIngredient = consumables?.find(
      (c) =>
...
export function canCook(camping: Camping): boolean {
  const hasPreparedCampsite =
    camping.campingActivities.find(
      (a) =>
        a.activity === "Prepare Campsite" &&
        (a.result === "success" ||
          a.result === "criticalSuccess" ||
          a.result === "failure"),
    ) !== undefined;
  const actorIsCooking =
    camping.campingActivities.find(
      (a) => a.activity === "Cook Meal" && a.actorUuid,
    ) !== undefined;
  return hasPreparedCampsite && actorIsCooking;
}

export function calculateConsumedFood(
  canCook: boolean,
  actorConsumables: FoodAmount,
  foodCost: FoodCost,
): ConsumedFood {
  ...
  const recipeSpecialIngredientCost = canCook
    ? foodCost.recipeSpecialIngredientCost
    : 0;
  const recipeBasicIngredientCost = canCook
    ? foodCost.recipeBasicIngredientCost
    : 0;
  const requiredRationsTotal = actorsConsumingMeals + actorsConsumingRations;
  const availableRationsTotal =
    availableRations + availableSubsistence + availableMagicalSubsistence;

  const consumedRations = Math.max(
    0,
    requiredRationsTotal - availableSubsistence - availableMagicalSubsistence,
  );
  const consumedSubsistence = Math.max(
    0,
    requiredRationsTotal - consumedRations - availableMagicalSubsistence,
  );
  const consumedMagicalSubsistence = Math.max(
    0,
    requiredRationsTotal - consumedSubsistence - consumedRations,
  );
  return {
    ...
    specialIngredients: {
      ...
      warning:
        recipeSpecialIngredientCost * actorsConsumingMeals >
        availableSpecialIngredients,
    },
    basicIngredients: {
      value: recipeBasicIngredientCost * actorsConsumingMeals,
      warning:
        recipeBasicIngredientCost * actorsConsumingMeals >
...
export function getIngredientList(
  ...
  const result = [];
  if (basicIngredients)
    result.push(`<b>Basic Ingredients</b>: ${basicIngredients}`);
  if (specialIngredients)
    result.push(`<b>Special Ingredients</b>: ${specialIngredients}`);
  return result.map((a) => `<li>${a}</li>`).join("");
}
...
export async function addDiscoverSpecialMealResult(
  ...
  if (campingActor) {
    ...
    const itemsToAdd = (await getItemsByUuid(new Set(critFailUuids))).map((i) =>
      i.toObject(),
    );
    ...
    const content = `<p>Removed:</p>
          ...
          recipe || itemsToAdd.length > 0
            ...
            <ul>
                ...
                ${
                  itemsToAdd.length === 0
                    ? ""
                    : `<li><b>Meal Effects</b>: ${itemsToAdd
                        .map((i) => i.name)
                        .join(", ")}</li>`
                }
            </ul>
        ...
        `;
    if (recipe) {
      camping.cooking.knownRecipes = Array.from(
        new Set([...camping.cooking.knownRecipes, recipe]),
      );
...
export async function addHuntAndGatherResult(
  game: Game,
  actorAndIngredients: ActorAndIngredients,
): Promise<void> {
  const campingActor = getCampingActor(game);
  if (campingActor) {
    const camping = getCamping(campingActor);
    const actor =
      (await getHuntAndGatherActor(camping)) || actorAndIngredients.actor;
    const specialIngredients = actorAndIngredients.specialIngredients;
    const basicIngredients = actorAndIngredients.basicIngredients;
    await addFood([actor], {
      specialIngredients,
    ...
    const content = `<p>Added to ${addOf(
...
export function getCookingActorUuid(data: Camping): string | null {
  return (
    data.campingActivities.find((a) => a.activity === "Cook Meal")?.actorUuid ??
    null
  );
}

export async function getCookingActorByUuid(
  data: Camping,
): Promise<Actor | null> {
...
export function getChosenMealData(data: Camping): RecipeData {
  ...
  const chosenMeal = knownRecipes.includes(data.cooking.chosenMeal)
    ? data.cooking.chosenMeal
    : "Basic Meal";
  const recipeData = getRecipeData(data);
  return (
    recipeData.find((a) => a.name === chosenMeal) ??
...
export async function subsist(
  game: Game,
  actor: Actor,
  region: string,
): Promise<void> {
  const { zoneDC } = getRegionInfo(region);
