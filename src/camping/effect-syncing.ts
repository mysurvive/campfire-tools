...
export async function syncActorMealEffects(
  ...
  await Promise.all(
    options.actors
      ...
      .map((a) =>
        addActorRecipeEffect(
          a,
          options.recipe,
          options.degree,
          options.actorUuidAndFavoriteMeal.get(a.uuid),
        ),
      ),
  );
}
...
function getActivityOutcomeUuids(
...
export function getCampingEffectUuids(activity: CampingActivityData): string[] {
  return [
    ...
    ...(activity.effectUuids === undefined
      ? []
      : activity.effectUuids.map((e) => e.uuid)),
  ];
}

export function getAllCampingEffectUuids(
...
function getActivityEffectsByResult(
  data: CampingActivityData | undefined,
  result: StringDegreeOfSuccess | null,
): ActivityEffect[] {
  return [
    ...(data?.effectUuids ?? []),
...
function getEffectUuidsForActors(
  options: SyncActorCampingEffectOptions,
): EffectsForActor[] {
  const actorActivityDataByName = groupBySingle(
    options.campingActivities,
    (a) => a.name,
  );
  const allEffects = Array.from(options.actorCampingActivities.values()).map(
    (e) => {
  ...
  allEffects.forEach((e) => {
    ...
    const uuids = e.effects
      .filter((e) => e.targetAll === false)
      .map((e) => e.uuid);
    effectsForActors.set(actorUuid, [
      ...(effectsForActors.get(actorUuid) ?? []),
      ...uuids,
    ]);
  });
  return options.actors.map((a) => {
    return {
      actor: a,
      effectUuids: [
        ...effectsUuidsForAllActors,
        ...(effectsForActors.get(a.uuid) ?? []),
      ],
    };
  });
}

async function syncActorCampingEffects(
  actor: Actor,
  effects: (Item & EffectItem)[],
  allUuids: Set<string>,
): Promise<void> {
  ...
  const effectsToRemove = existingEffects
    .filter((e) => !effectNames.has(e.name))
    .map((e) => e.id);
  await Promise.all([
    actor.createEmbeddedDocuments(
      "Item",
      effectsToAdd.map((e) => e.toObject()),
    ),
    actor.deleteEmbeddedDocuments("Item", effectsToRemove),
  ]);
}

export async function syncActorsCampingEffects(
  options: SyncActorCampingEffectOptions,
): Promise<void> {
  ...
  await Promise.all(
    effectUuidsForActors
      .filter((a) => a.actor.type === "npc" || a.actor.type === "character")
      .map(async (effectsForActor) => {
        const effects = await getEffectsByUuid(
          new Set(effectsForActor.effectUuids),
        );
...
export abstract class DiffListener {
  ...
  async fireChange(): Promise<void> {
    ...
    if (isFirstGm(this.game)) {
      ...
      this.game.socket?.emit("module.campfire-tools", {
        action: this.action,
      });
    }
  }
  ...
  protected abstract shouldFireChange(
    previous: Camping,
    update: Partial<Camping>,
  ): boolean;
}
...
export class CampingActivitiesListener extends DiffListener {
  ...
  protected shouldFireChange(
    previous: Camping,
    update: Partial<Camping>,
  ): boolean {
    const diff = diffObject(previous, update) as Partial<Camping>;
    return (
      "campingActivities" in diff &&
      update?.campingActivities?.find(
        (a) =>
  ...
  override async onReceive(camping: Camping): Promise<void> {
      ...
      await Promise.all(
        camping.campingActivities
          ...
          .map(async (a) => {
            ...
            return [a.activity, result] as [
              CampingActivityName,
              CampingActivityResult,
            ];
          }),
      )
    ).filter(([, result]) => result.actor !== null);
    const actorCampingActivities = new Map<
      CampingActivityName,
      CampingActivityResult
    >(activities);
...
export async function eat(camping: Camping): Promise<void> {
  const actorUuidAndMeal = groupBySingle(
  ...
  if (degree !== null && recipe !== undefined) {
    const actorUuidsEatingMeal = new Set(
      camping.cooking.actorMeals
