...
async function succeededEncounterFlatCheck(
  region: string,
  dc: EncounterDc,
): Promise<boolean> {
  ...
  const degreeOfSuccess = determineDegreeOfSuccess(
    dieRoll.total,
    dieRoll.total,
    dc.total,
  );
...
export async function rollRandomEncounter(
  ...
  region: string,
  dc: EncounterDc,
  forgoFlatCheck = false,
): Promise<boolean> {
  const creatureRollTable = (await findRollTableUuidWithFallback(
    game,
    region,
    "campfire-tools.kingmaker-tools-random-encounters",
  ))!;
  const worldEncounterTable = getStringSetting(
    game,
    "proxyEncounterTable",
  )?.trim();
  const encounterTypeRollTable = findWorldTableUuid(game, worldEncounterTable)!;
  const rollMode = getStringSetting(
    game,
    "randomEncounterRollMode",
  ) as unknown as keyof CONFIG.Dice.RollModes;
  const rollOptions: Partial<RollTable.DrawOptions> = { rollMode };

  if (forgoFlatCheck || (await succeededEncounterFlatCheck(region, dc))) {
    if (
      encounterTypeRollTable === undefined ||
      encounterTypeRollTable === null ||
      encounterTypeRollTable.trim() === ""
    ) {
      console.log("Rolling on creature table " + creatureRollTable);
      await rollRollTable(creatureRollTable, rollOptions);
    } else {
      const typeResult = await rollRollTable(
        encounterTypeRollTable,
        rollOptions,
      );
...
export function getEncounterDC(data: Camping, game: Game): EncounterDc {
  ...
  const activityModifier = getCampingActivityData(data)
    .map((campingActivityData) => {
      const campingActivity = data.campingActivities.find(
        (ca) => ca.activity === campingActivityData.name,
      );
      const result = campingActivity?.result ?? null;
      const actorUuid = campingActivity?.actorUuid ?? null;
      if (
        actorUuid !== null &&
        (result !== null || campingActivityData.skills.length === 0)
      ) {
