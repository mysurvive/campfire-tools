...
function calculateRestSeconds(
  ...
  if (partySize < 2) {
    ...
    let seconds =
      restDurationSeconds + Math.floor(restDurationSeconds / (partySize - 1));
...
async function healMoreHp(actor: Actor, data: Camping): Promise<void> {
  const healMoreHp = await hasItemByUuid(
    actor,
    "effect",
    new Set([
      getRecipeData(data).find((r) => r.name === "Basic Meal")!.criticalSuccess!
        .effects![0]!.uuid!,
      /*getCampingActivityData(data).find(
        (a) => a.name === "Dawnflower's Blessing",
      )!.effectUuids![0]!.uuid!,*/
    ]),
  );
  if (healMoreHp) {
    ...
    if (hpRestored > 0) {
      await actor.update({
        "system.attributes.hp.value": currentHp + hpRestored,
      });
    }
  }
}

async function removeExpiredMealEffects(
  actors: Actor[],
  camping: Camping,
): Promise<void> {
...
async function getAverageRestDuration(camping: Camping): Promise<number> {
  ...
  const fishOnAStick = getRecipeData(camping).find(
    (r) => r.name === "Fish-On-A-Stick",
  )!;
  ...
  const actorsFavoriteMeal = (
    await Promise.all(
      actors.map(async (a) =>
        (await actorHasEffectByUuid(a, favoriteMeal)) ? 1 : 0,
      ),
    )
  ).reduce((a: number, b: number) => a + b, 0);
  const actorsCriticalFailure = (
    await Promise.all(
      actors.map(async (a) =>
        (await actorHasEffectByUuid(a, criticalFailure)) ? 1 : 0,
      ),
    )
  ).reduce((a: number, b: number) => a + b, 0);
  const actorsHavingNeither =
    actors.length - (actorsCriticalFailure + actorsFavoriteMeal);
  return Math.floor(
    (8 * 3600 * actorsHavingNeither +
      7 * 3600 * actorsFavoriteMeal +
      9 * 3600 * actorsCriticalFailure) /
      actors.length,
  );
}

export function getActorsKeepingWatch(data: Camping, actors: Actor[]): number {
  const organizeWatchCritSuccess = data.campingActivities.find(
    (a) => a.activity === "Organize Watch" && a.result === "criticalSuccess",
  );
  // 1 person can't keep watch, so don't increase to 2
  const additionalWatchers =
    actors.length > 1 ? (organizeWatchCritSuccess ? 1 : 0) : 0;
  const blacklistedActorNumber = data.actorUuidsNotKeepingWatch.filter((uuid) =>
    data.actorUuids.includes(uuid),
  ).length;
  return (
    actors.length +
    additionalWatchers +
    data.increaseWatchActorNumber -
    blacklistedActorNumber
  );
}

export async function getWatchSecondsDuration(
  actors: Actor[],
  data: Camping,
): Promise<number> {
  return calculateRestSeconds(
    getActorsKeepingWatch(data, actors),
    await getAverageRestDuration(data),
  );
}
...
export async function getWatchRandomEncounterData(
  ...
  const dailyPrepsDuration = calculateDailyPreparationsSeconds(
    camping.gunsToClean,
  );
...
export async function rest(params: RestParameters): Promise<void> {
  ...
  if (camping.watchSecondsRemaining === 0) {
    camping.watchSecondsRemaining = await startWatch(
      game,
      camping,
...
async function startWatch(
  ...
  const { advanceToSeconds, remainingSeconds, encounter } =
    await getWatchRandomEncounterData(game, camping, watchDurationSeconds);
  await game.time.advance(advanceToSeconds);
  if (encounter) {
    const actorsKeepingWatch = actors.filter(
      (a) =>
        !camping.actorUuidsNotKeepingWatch.includes(a.uuid) &&
        (a.type === "character" || a.type === "npc"),
    );
    if (actorsKeepingWatch.length > 0) {
      const actorKeepingWatchIndex = Math.floor(
        Math.random() * actorsKeepingWatch.length,
      );
      const chosenActor = actorsKeepingWatch[actorKeepingWatchIndex] as Actor;
...
async function completeDailyPreparations(
  ...
  await removeActorsEffectsByName(
    actors,
    new Set(["Undead Guardians (Aided)", "Undead Guardians (Defended)"]),
  );
  await removeActorsEffectsByUuid(
    actors,
    new Set([
      "Compendium.campfire-tools.kingmaker-tools-camping-effects.Item.ZKJlIqyFgbKDACnG", // enhance weapons
      "Compendium.campfire-tools.kingmaker-tools-camping-effects.Item.pbhKANmDOvwuuchk", // aided
      "Compendium.campfire-tools.kingmaker-tools-camping-effects.Item.tERAC9KHwBjoUt5u", // defended
    ]),
  );
