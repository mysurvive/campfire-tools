...
export class CampingSheet extends FormApplication<
  CampingOptions & FormApplicationOptions,
  ViewCampingData,
  null
> {
  ...
  static override get defaultOptions(): FormApplicationOptions {
    ...
    options.template = "modules/campfire-tools/templates/camping/sheet.hbs";
    ...
    options.dragDrop = [
      {
        dropSelector:
          ".new-camping-actor, .camping-activity, .camping-actor, .eating-actor",
  ...
  override async getData(): Promise<ViewCampingData> {
    ...
    const { total: encounterDC, modifier: currentEncounterDCModifier } =
      getEncounterDC(data, this.game);
    ...
    const viewActors = await toViewActors(
      data.actorUuids,
      data.campingActivities,
      getCampingActivityData(data),
      isUser,
    );
    const dailyPrepsSeconds = calculateDailyPreparationsSeconds(
      data.gunsToClean,
    );
    const viewData: ViewCampingData = {
      ...
      adventuringSince: formatHours(
        sumElapsedSeconds,
        data.dailyPrepsAtTime > currentSeconds,
      ),
      regions: Array.from(regions.keys()),
      currentRegion,
      actors: viewActors,
      campingActivities: await toViewCampingActivities(
        data.campingActivities,
        activityData,
        new Set(data.lockedActivities),
      ),
      ...
      timeMarkerPositionPx:
        Math.floor(getTimeOfDayPercent(getWorldTime(this.game)) * 8.46) - 4,
      hasCookingActor: canCook(data),
      consumedFood: calculateConsumedFood(canCook(data), actorConsumables, {
        actorsConsumingRations: data.cooking.actorMeals.filter(
          (a) => a.chosenMeal === "rationsOrSubsistence",
        ).length,
        actorsConsumingMeals: data.cooking.actorMeals.filter(
          (a) => a.chosenMeal === "meal",
        ).length,
      ...
      actorMeals: await toViewActorMeals(
        data.actorUuids,
        data.cooking.actorMeals,
        getRecipeData(data),
      ),
  ...
  private async getCookingSkillData(data: Camping): Promise<{
    ...
    const cookingSkill = hasCookingLore
      ? data.cooking.cookingSkill
      : "survival";
    return {
      cookingSkills,
      cookingSkill,
  ...
  private async getMealDc(
    data: Camping,
    chosenMealData: RecipeData,
  ): Promise<number> {
  ...
  private emit(args: object): void {
    this.game.socket!.emit("module.campfire-tools", args);
  }
  ...
  override activateListeners(html: JQuery): void {
    ...
    const $html = html[0];
    listenClick($html as HTMLElement, ".remove-actor", async (ev) => {
    ...
    listenClick($html as HTMLElement, ".post-combat-effects", async () => {
      const current = await this.read();
      await combatEffectsToChat(current);
    });
    listenClick($html as HTMLElement, ".advance-hours", async (ev) => {
      const target = ev.currentTarget as HTMLButtonElement;
      const hours = target.dataset["hours"] ?? "0";
      await this.advanceHours(parseInt(hours, 10));
    });
    listenClick($html as HTMLElement, ".manage-recipes", async () => {
      const current = await this.read();
      await manageRecipesDialog({
        ...
        onSubmit: async (knownRecipes, deletedRecipes) => {
          ...
          current.cooking.homebrewMeals = current.cooking.homebrewMeals.filter(
            (m) => !deletedRecipes.has(m.name),
          );
          await this.update({ cooking: current.cooking });
        },
      });
    });
    listenClick($html as HTMLElement, ".add-recipes", async () => {
      const current = await this.read();
    ...
    listenClick($html as HTMLElement, ".unlock-activities", async () => {
      const current = await this.read();
      await manageActivitiesDialog({
        ...
        onSubmit: async (lockedActivities, deletedActivities) => {
          await this.update({
            lockedActivities: Array.from(lockedActivities).filter(
              (a) => !deletedActivities.has(a),
            ),
            homebrewCampingActivities: current.homebrewCampingActivities.filter(
              (a) => !deletedActivities.has(a.name),
            ),
            campingActivities: current.campingActivities.filter(
              (a) => !deletedActivities.has(a.activity),
            ),
          });
        },
      });
    });
    listenClick($html as HTMLElement, ".add-activity", async () => {
      const current = await this.read();
        ...
        onSubmit: async (activity) => {
          await this.update({
            homebrewCampingActivities: [
              ...current.homebrewCampingActivities,
              activity,
            ],
          });
        },
    ...
    listenClick($html as HTMLElement, ".subsist", async (ev) => {
      const button = ev.currentTarget as HTMLButtonElement;
      const uuid = button.dataset["uuid"]!;
      const current = await this.read();
    ...
    listenClick(
      $html as HTMLElement,
      ".roll-encounter",
      async () => await this.rollRandomEncounter(true),
    );
    listenClick(
      $html as HTMLElement,
      ".check-encounter",
      async () => await this.rollRandomEncounter(),
    );
    listenClick(
      $html as HTMLElement,
      ".consume-food",
      async () => await this.consumeFood(),
    );
    listenClick(
      $html as HTMLElement,
      ".decrease-zone-dc-modifier",
      async () => {
        const current = await this.read();
    ...
    listenClick(
      $html as HTMLElement,
      ".eat-food",
      async () => await eat(await this.read()),
    );
    listenClick(
      $html as HTMLElement,
      ".increase-zone-dc-modifier",
      async () => {
        const current = await this.read();
        await this.update({ encounterModifier: current.encounterModifier + 1 });
      },
    ...
    listenClick(
      $html as HTMLElement,
      ".reset-zone-dc-modifier",
      async () => await this.update({ encounterModifier: 0 }),
    );
    listenClick($html as HTMLElement, ".cook-food", async () => {
      const current = await this.read();
      ...
      if (cookingActor) {
        ...
        const activity = getCampingActivityData(current).find(
          (a) => a.name === "Cook Meal",
        )!;
    ...
    listenClick($html as HTMLElement, ".rest", async () => {
    ...
    listenClick($html as HTMLElement, ".roll-check", async (ev) => {
      const button = ev.currentTarget as HTMLButtonElement;
      const activity = button.dataset["activity"]!;
      const skill = button.dataset["skill"]!;
      await this.rollCheck(activity, skill);
    });
    listenClick($html as HTMLElement, ".recipe-info", async () => {
      const current = await this.read();
      const recipe = getChosenMealData(current);
      const item = (await fromUuid(recipe.uuid)) as Item | null;
      item?.sheet?.render(true);
    });
    listenClick($html as HTMLElement, ".camping-settings", async () => {
      const current = await this.read();
      ...
      campingSettingsDialog({
        data: {
          ...
          actorsKeepingWatch: actors.map((a) => {
            return {
              uuid: a.uuid,
  ...
  private async rollCheck(activity: string, skill: string): Promise<void> {
    const current = await this.read();
    const activityData = getCampingActivityData(current).find(
      (a) => a.name === activity,
    );
    const actorUuid = current.campingActivities.find(
      (a) => a.activity === activity,
    )?.actorUuid;
    const actor = actorUuid
      ? ((await fromUuid(actorUuid)) as Actor | null)
      : null;
    if (activityData && actor) {
      if (activity === "Discover Special Meal") {
        await this.discoverSpecialMeal(
          activityData,
          skill,
          actor,
  ...
  private async rollStandardCheck(
    ...
    if (dcType) {
      ...
      await this.setActivityResult(
        activityName,
        await rollCampingCheck(rollOptions),
      );
    } else {
      askDcDialog({
  ...
  private async removeActor(uuid: string, type: string): Promise<void> {
    ...
    if (type === "camping-actors") {
      ...
      const activity = campingConfiguration.campingActivities.find(
        (a) => a.activity === type,
      );
      if (activity) {
        activity.actorUuid = null;
        await this.update({
          campingActivities: campingConfiguration.campingActivities,
        });
      }
    }
  }
  ...
  protected override _onDragStart(event: DragEvent): void {
    ...
    event?.dataTransfer?.setData(
      "text/plain",
      JSON.stringify({ type: "Actor", uuid: uuid }),
    );
  }
  ...
  protected override async _onDrop(event: DragEvent): Promise<void> {
    ...
    if (document) {
      const campingConfiguration = await this.read();
      if (
        target.classList.contains("camping-activity") &&
        (document.type === "character" || document.type === "npc")
      ) {
        ...
        const proficiencyRequirements =
          data.find((a) => a.name === activityName)?.skillRequirements ?? [];
        if (document instanceof Actor) {
          try {
            if (!current.ignoreSkillRequirements) {
              await validateSkillProficiencies(
                document,
                proficiencyRequirements,
              );
            }
            await this.setActivityActor(
        ...
        target.classList.contains("new-camping-actor") &&
        !campingConfiguration.actorUuids.includes(document.uuid)
      ) {
        await this.update({
          actorUuids: [...campingConfiguration.actorUuids, document.uuid],
        });
      } else if (
        target.classList.contains("camping-actor") &&
        document instanceof Item
      ) {
        await this.handleItemDrop(document, target.dataset["actorUuid"]!);
      } else if (
        target.classList.contains("eating-actor") &&
        document instanceof Item
      ) {
        await this.handleItemDrop(document, target.dataset["actorUuid"]!);
      }
  ...
  private async setActivityActor(
    campingConfiguration: Camping,
  ...
  private async parseDropData(event: DragEvent): Promise<Actor | Item | null> {
    ...
    try {
      ...
      if (document) {
        ...
        if (
          (document instanceof Actor && allowedActors.has(type)) ||
          document instanceof Item
        ) {
          return document;
        } else {
          console.error(
            "No character or item document type, instead found: " + type,
          );
          return null;
        }
      }
  ...
  private async openUuidJournal(uuid: string): Promise<void> {
    const journal = (await fromUuid(uuid)) as
      | JournalEntry
      | JournalEntryPage
      | null;
  ...
  protected async _updateObject(
    event: Event,
    formData: CampingData,
  ): Promise<void> {
    console.log("formdata", formData);
    console.log("event", event);
    const current = await this.read();
    const mealDegreeOfSuccess = this.parseNullableSelect(
      formData.mealDegreeOfSuccess,
    ) as StringDegreeOfSuccess | null;
    const update = {
      ...
      campingActivities: getCampingActivityData(current).map((activityData) => {
        ...
        return {
          ...
          result: this.parseNullableSelect(
            data[degreeKey],
          ) as StringDegreeOfSuccess | null,
          actorUuid:
            current.campingActivities.find(
              (activity) => activity.activity === activityData.name,
            )?.actorUuid ?? null,
        };
      }),
      cooking: {
  ...
  private async consumeFood(): Promise<void> {
    ...
    const actorConsumables = await getActorConsumables(actors);
    const chosenMealData = getRecipeData(current).find(
      (a) => a.name === current.cooking.chosenMeal,
    );
    const consumed = calculateConsumedFood(canCook(current), actorConsumables, {
      actorsConsumingRations: current.cooking.actorMeals.filter(
        (a) => a.chosenMeal === "rationsOrSubsistence",
      ).length,
      actorsConsumingMeals: current.cooking.actorMeals.filter(
        (a) => a.chosenMeal === "meal",
      ).length,
  ...
  private async parseActorMeals(formData: CampingData): Promise<ActorMeal[]> {
    ...
    return current.actorUuids.map((uuid) => {
      const x: ActorMeal = {
        actorUuid: uuid,
        favoriteMeal: this.parseNullableSelect(
          data["actorFavoriteMeal." + uuid],
        ),
  ...
  private async setActivityResult(
    activityName: string,
    degreeOfSuccess: DegreeOfSuccess | null,
  ): Promise<void> {
    const current = await this.read();
    const activity = current.campingActivities.find(
      (a) => a.activity === activityName,
    );
  ...
  private async discoverSpecialMeal(
    ...
    const current = await this.read();
    const availableFood = await getActorConsumables(
      await this.getActors(current),
    );
    const availableRecipes = getRecipesKnownInRegion(
      currentRegion,
      recipeData,
        ...
        if (recipeName !== null && data) {
          ...
          if (result !== null) {
            const isCriticalSuccess =
              result === DegreeOfSuccess.CRITICAL_SUCCESS;
            const isSuccess = result === DegreeOfSuccess.SUCCESS;
            const isCriticalFailure =
              result === DegreeOfSuccess.CRITICAL_FAILURE;
            const removeIngredients = {
              specialIngredients: isCriticalSuccess
                ? data.specialIngredients
                : data.specialIngredients * 2,
              basicIngredients: isCriticalSuccess
                ? data.basicIngredients
                : data.basicIngredients * 2,
              rations: 2,
            };
            const recipeToAdd =
              isSuccess || isCriticalSuccess ? data.name : null;
            const critFailUuids = isCriticalFailure
              ? data.criticalFailure.effects?.map((e) => e.uuid) ?? []
              : [];
            await postDiscoverSpecialMealResult(
              actor,
              removeIngredients,
              recipeToAdd,
              critFailUuids,
            );
            await this.persistActivityDegreeOfSuccess(
              current,
              result,
  ...
  private async huntAndGather(
    ...
    const result = await rollCampingCheck({
      ...
      activity,
      dc: activity.dc,
    } as SkillCheckOptions);
    if (result !== null) {
      const ingredients = await huntAndGather(result, currentRegion);
    ...
    setupDialog(game, "Camping", "yybLhORz4PeZxCp0", async () => {
      ...
      const migratedRecipes =
        game?.actors
          ...
          ?.flatMap((a) => {
            const knownRecipes = a.getFlag("campfire-tools", "knownRecipes");
      ...
      await sheetActor?.setFlag(
        "campfire-tools",
        "camping-sheet",
        getDefaultConfiguration(game, migratedRecipes),
      );
