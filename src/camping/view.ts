...
async function toViewActivity(
  ...
  return {
    ...
    selectedSkill:
      (activity?.selectedSkill as string) ??
      (activityDataSkills.length > 0 ? activityDataSkills[0] : null),
    isHidden,
...
export async function toViewActor(uuid: string): Promise<ViewActor> {
  ...
  return {
    image: actor?.img ?? "modules/campfire-tools/static/img/add-actor.webp",
    uuid,
  };
}

function getCssClass(
  chosenActivity: CampingActivity | undefined,
...
export async function toViewActors(
  ...
  const actorInfos = await Promise.all(
    actorUuids.map((uuid) => toViewActor(uuid)),
  );
  return actorInfos
    .filter((a) => a !== null)
    .map((actor) => {
      ...
      const hasSkillCheck =
        (
          activityData.find(
            (a) =>
              a.name === chosenActivity?.activity &&
...
export async function toViewCampingActivities(
  ...
  const prepareCampActivity = activities.find(
    (a) => a.activity === "Prepare Campsite",
  );
  const prepareCampDegreeOfSuccess = prepareCampActivity?.result ?? null;
  const canPerformActivities =
    prepareCampDegreeOfSuccess !== "criticalFailure" &&
    prepareCampDegreeOfSuccess !== null;
  const result = await Promise.all(
    data.map((a) => {
      const isHidden =
        (lockedActivities.has(a.name) || !canPerformActivities) &&
        a.name !== "Prepare Campsite";
      return toViewActivity(
        activities.find((d) => d.activity === a.name),
        a,
        isHidden,
      );
    }),
  ...
  result.sort(
    (a, b) =>
      (a.name === "Prepare Campsite" ? -1 : 0) -
        (b.name === "Prepare Campsite" ? -1 : 0) ||
...
export async function toViewActorMeals(
  actorUuids: string[],
  actorMeals: ActorMeal[],
  recipeData: RecipeData[],
): Promise<ViewActorMeal[]> {
  return await Promise.all(
    actorUuids.map(async (uuid) => {
      const favoriteMeal =
        actorMeals.find((m) => m.actorUuid === uuid)?.favoriteMeal ?? null;
      const favoriteMealUuid = favoriteMeal
        ? recipeData.find((r) => r.name === favoriteMeal)?.uuid ?? null
        : null;
      return {
        ...
        favoriteMealUuid,
        chosenMeal:
          actorMeals.find((m) => m.actorUuid === uuid)?.chosenMeal ?? "meal",
      };
    }),
  );
}
