...
export async function findRollTableUuidWithFallback(
  game: Game,
  tableName: string,
  compendiumName = "campfire-tools.kingmaker-tools-rolltables",
): Promise<string | undefined> {
  const worldUuid = findWorldTableUuid(game, tableName);
  const compendiumUuid = await findCompendiumTableUuid(
    game,
    tableName,
    compendiumName,
  );
  ...
  console.log(
    `Looking up ${tableName} in ${compendiumName}, world: ${worldUuid}, compendium: ${compendiumUuid}`,
  );
...
export function getWorldTableUuidMappings(game: Game): Record<string, string> {
  ...
  return Object.fromEntries(
    tables.map((t: RollTable) => [`RollTable.${t.id!}`, t.name!]),
  );
}

export function findWorldTableUuid(
  game: Game,
  tableName: string,
): string | undefined {
  return (
    Object.entries(getWorldTableUuidMappings(game))
...
export async function buildUuids(
  ...
  return Object.fromEntries(
    documents.map((document) => [
      document.name,
      `Compendium.${compendiumName}.${document.id}`,
    ]),
  );
}
