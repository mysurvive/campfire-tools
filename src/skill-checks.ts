async function rollSkillCheck(
  ...
  for (const actor of actors) {
    const check =
      skill === "perception" ? actor.perception : actor.skills[skill];
    await check?.roll({
      rollMode: "gmroll",
      dc: checkDc,
    });
...
export async function rollExplorationSkillCheck(
  ...
  const actorsWithEffectApplied = actors.filter((actor) =>
    getActiveExplorationEffects(actor).has(effect),
  );
      ...
      buttons: {
        roll: {
          ...
          callback: async (html): Promise<void> => {
            ...
            const skillInput = $html.querySelector(
              "select[name=skill]",
            ) as HTMLSelectElement;
            const dcInput = $html.querySelector(
              "input[name=dc]",
            ) as HTMLInputElement;
