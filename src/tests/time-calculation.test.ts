...
import { formatWorldDateTime } from "../time/format";
...
describe("time of day calculation", () => {
  ...
  test("retracting from 12:00:00 to 11:00:01", () => {
    ...
    expect(time.diffSeconds(noon, TimeChangeMode.RETRACT)).toBe(
      (59 * 60 + 59) * -1,
    );
  });
  ...
  test("advancing from 12:00:00 to 11:59:59", () => {
    ...
    expect(time.diffSeconds(noon, TimeChangeMode.ADVANCE)).toBe(
      23 * 60 * 60 + 59 * 60 + 59,
    );
  });
