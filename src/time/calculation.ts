...
export function getWorldTime(game: Game): DateTime {
  const value = game.settings.get(
    "pf2e",
    "worldClock.worldCreatedOn",
  ) as string;
  return DateTime.fromISO(value).toUTC().plus({ seconds: game.time.worldTime });
}
...
export class TimeUnit implements TimeDiff {
  ...
  diffSeconds(worldTime: DateTime, mode: TimeChangeMode): number {
    const targetTime =
      mode == TimeChangeMode.ADVANCE
        ? worldTime.plus(this.unit)
        : worldTime.minus(this.unit);
