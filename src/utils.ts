...
export interface DegreeOfSuccessMessageConfig {
  critSuccess?: string | undefined;
  success?: string | undefined;
  failure?: string | undefined;
  critFailure?: string | undefined;
  isPrivate?: boolean;
}
...
export function uncapitalize(word: string): string {
  if (word[0]) return word[0].toLowerCase() + word.substring(1);
  else return "";
}

export function unpackFormArray<T>(
...
/*export function mergeObjects<
  A extends Record<string, V>,
  B extends Record<string, V>,
  V,
>(obj1: A, obj2: B, conflictFunction: (a: V, b: V) => V): Record<string, V> {
...
export function groupBySingle<T, R>(
  array: T[],
  criterion: (value: T) => R,
): Map<R, T> {
  return new Map(array.map((elem) => [criterion(elem), elem]));
}
...
export function parseRadio($html: HTMLElement, name: string): string | null {
  const input = Array.from(
    $html.querySelectorAll(`input[name="${name}"]`),
  ) as HTMLInputElement[];
