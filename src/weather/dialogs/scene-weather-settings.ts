      ...
      content: `
        <form class="simple-dialog-form">
            <div>
                ...
                <input type="checkbox" name="sync-weather" id="km-sync-weather" ${
                  syncWeather ? "checked" : ""
                }>
            </div>
            <div>
                <label for="km-sync-weather-playlist">Sync Weather Playlist</label>
                <input type="checkbox" name="sync-weather-playlist" id="km-sync-weather-playlist" ${
                  syncWeatherPlaylist ? "checked" : ""
                }>
            </div>
        </form>
        `,
      buttons: {
        save: {
          ...
          callback: async (html): Promise<void> => {
            ...
            const syncWeatherPlaylist = parseCheckbox(
              $html,
              "sync-weather-playlist",
            );
