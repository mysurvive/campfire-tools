...
import { eventLevels, getSeason, AedenirMonth } from "../camping/regions";
...
async function rollOnWeatherEventTable(
  ...
  const { table, draw } = await rollRollTable(
    uuids["Weather Events"] as string,
    { rollMode, displayChat: false },
  );
  const { results } = draw;
  ...
  const eventLevel =
    Array.from(eventLevels.entries())
      .filter(([name]) => event.startsWith(name))
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      .map(([_, level]) => level)[0] ?? 0;
  if (eventLevel > averagePartyLevel + weatherHazardRange) {
    console.info(
      `Re-rolling event, level ${event.level} is more than ${weatherHazardRange} levels higher than party level ${averagePartyLevel}`,
    );
    await rollOnWeatherEventTable(
      game,
      averagePartyLevel,
      weatherHazardRange,
      rollMode,
...
async function rollWeatherEvent(
  ...
  const { isSuccess, total } = await rollCheck(
    17,
    "Rolling for weather event with DC 17",
    rollMode,
  );
  if (total === 20) {
    await rollOnWeatherEventTable(
...
async function rollWeather(
  ...
  const month = SimpleCalendar.api.getCurrentMonth()?.name;
  console.log(month);
  const { precipitationDC, coldDC } = getSeason(month as AedenirMonth);
