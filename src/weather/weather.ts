...
function getCurrentlyPlayingWeather(game: Game): StoredDocument<Playlist>[] {
  return (
    game?.playlists?.filter(
      (p) => (p.name?.startsWith("weather.") ?? false) && p?.playing,
    ) ?? []
  );
}

async function applyWeatherSound(
  game: Game,
  effectName: WeatherEffectName,
): Promise<void> {
  const playlistName = `weather.${effectName}`;

  // stop all others playlists
  for (const p of getCurrentlyPlayingWeather(game).filter(
...
function getWeatherPlaylist(
  game: Game,
  activeScene: Scene | undefined | null,
): WeatherEffectName {
...
async function syncSceneWeather(
  game: Game,
  scene: Scene | null | undefined,
): Promise<void> {
...
async function syncScenePlaylist(
  game: Game,
  scene: Scene | null | undefined,
): Promise<void> {
...
export async function setWeather(
  ...
  await game.settings.set("campfire-tools", "currentWeatherFx", effectName);
...
export async function toggleSheltered(game: Game): Promise<void> {
  ...
  await game.settings.set("campfire-tools", "enableSheltered", isEnabled);
  await syncWeather(game);
}

export async function toggleWeather(game: Game): Promise<void> {
  const isEnabled = !getBooleanSetting(game, "enableWeather");
  await game.settings.set("campfire-tools", "enableWeather", isEnabled);
  await syncWeather(game);
}

export async function onPreUpdateScene(
  game: Game,
  scene: Scene,
  ...
  if (
    isFirstGm(game) &&
    getBooleanSetting(game, "enableWeather") &&
    update?.active
  ) {
...
export async function onUpdateScene(
  game: Game,
  scene: Scene,
  update: Partial<Scene>,
): Promise<void> {
